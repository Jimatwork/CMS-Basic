...
function tj(userId) {
	...
	$.ajax({
		type: "post",
		url: getRequestIp1() + "updateUserInfo",
		data: fo,
		dataType: "json",
		async: true,
...
function showUserInfo(userId) {
	$.ajax({
		...
		$('.nick').val(data.nickName) //用户昵称
				$('.start_img').attr('src', data.homePageImg) //首页图片
				$('.start_img1').attr('src', data.homePageImg) //首页图片
				...
				$('.user_wa_img').attr('src', data.detailPageImg) //详情页图片
				$('.user_wa_img1').attr('src', data.detailPageImg) //详情页图片
