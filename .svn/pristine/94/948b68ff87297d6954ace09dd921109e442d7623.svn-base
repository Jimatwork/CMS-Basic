...
var uploader = WebUploader.create({
	...
	// 文件接收服务端。
	server: getRequestIp1() + 'uploadfile1',
	// 选择文件的按钮。可选。
...
uploader.on('uploadSuccess', function(file, object) {
	...
	if(url == '上传失败,视频不能大于两分钟！') {
		...
		var surl = url.substring(url.lastIndexOf('.'));
		if(surl == '.mp4' || surl == '.mpg' || surl == '.mpeg' || surl == '.avi' || surl == '.rm' || surl == '.rmvb' || surl == '.asf' || surl == '.dat') {
			$("#video_mytitle").html(url);
		}
//		console.log(111 + surl);
		//		$("#video_mytitle").html(url);
