...
public interface ColumnContentDao extends CrudRepository<ColumnContent, Integer> {
	...
	// 根据colId分页查询头条的所有信息
	@Query("from ColumnContent where state = 1 and (bih=1 or bih=:tag) and colId =:colId order by top desc ,create_date DESC")
	public List<ColumnContent> findByColId(@Param("colId") Long colId, @Param("tag") Integer tag, Pageable pageable);

	// 根据栏目colId查询所有
	@Query("from ColumnContent where state = 1 and (bih=1 or bih=:tag) and colId =:colId order by create_date DESC")
	public List<ColumnContent> findByColId(@Param("colId") Long colId, @Param("tag") Integer tag);

	// 根据用户appUserId 或 标题conTitle 模糊查询
	@Query(nativeQuery = true, value = " select * from column_content where app_user_id in (select a.id from app_user a  where user_name like :search_str) or con_title like :search_str or con_remark like :search_str and (is_public is null or is_public!=0)  and state =1 and (bih=1 or bih=:tag) order by top desc,create_date DESC limit :page,:size")
	public List<ColumnContent> getColumnContentByAppUserIdOrConTitle(@Param("search_str") String search_str,
			@Param("page") Integer page, @Param("size") Integer size, @Param("tag") Integer tag);

	// 获取模糊查询的总数
	@Query(nativeQuery = true, value = "select * from column_content where app_user_id in (select a.id from app_user a  where user_name like :search_str) or con_title like :search_str or con_remark like :search_str and (is_public is null or is_public!=0)  and state = 1 and (bih=1 or bih=:tag) order by create_date DESC")
	public List<ColumnContent> getColumnContentAll(@Param("search_str") String search_str, @Param("tag") Integer tag);
	...
	// 分页获取发现栏目的内容
	@Query("from ColumnContent where state =2 and (bih =1 or bih = :tag) and colId =:colId order by top desc , create_date desc")
	public List<ColumnContent> getFindManagers(@Param("colId") Long colId, @Param("tag") Integer tag,
			Pageable pageable);

	@Query("from ColumnContent where state =2 and (bih =1 or bih = :tag) and colId =:colId order by top desc,create_date DESC")
	public List<ColumnContent> getFindManagers(@Param("colId") Long colId, @Param("tag") Integer tag);

	@Query("from ColumnContent where state = 2 and (bih =1 or bih = :tag)  order by top desc , create_date desc")
	public List<ColumnContent> getFindManager(@Param("tag") Integer tag, Pageable pageable);

	@Query("from ColumnContent where state =2 and (bih =1 or bih = :tag)  order by top desc , create_date desc")
	public List<ColumnContent> getFindManager(@Param("tag") Integer tag);

	// 根据用户appUserId 或 标题conTitle 模糊查询
	@Query(nativeQuery = true, value = "select * from column_content where app_user_id in (select a.id from app_user a  where user_name like :search_str) or con_title like :search_str or con_remark like :search_str and (is_public is null or is_public!=0)  and state =2 and (bih =1 or bih = :tag) order by top desc,create_date DESC limit :page,:size")
	public List<ColumnContent> getFindColumnContent(@Param("search_str") String search_str, @Param("page") Integer page,
			@Param("size") Integer size, @Param("tag") Integer tag);

	// 获取模糊查询的总数
	@Query(nativeQuery = true, value = "select * from column_content where app_user_id in (select a.id from app_user a  where user_name like :search_str) or con_title like :search_str or con_remark like :search_str and (is_public is null or is_public!=0) and state =2 and (bih =1 or bih = :tag) order by create_date DESC")
